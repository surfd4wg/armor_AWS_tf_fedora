...
data "aws_ami" "fedora" {
  ...
  tags = merge(
        ...
        tomap(
          {"Zoo" = "AWS Zoofarm"
          "RESOURCE" = "fedora server"
          }
        )
        )
  filter {
    name   = "name"
    values = ["*Fedora-Cloud-Base-34-20210401.n.0.x86_64-hvm-us-east-1-gp2-0*"]
...
resource "aws_instance" "webserver" {
  ...
  user_data = "${file("install_userdata_fedora.sh")}"
